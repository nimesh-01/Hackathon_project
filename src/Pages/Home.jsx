    ...
    <div className="bg-transparent text-black font-sans">
      ...
      <section className="flex flex-col md:flex-row items-center justify-between p-6 md:p-12">
        <div className="w-full md:w-1/2 space-y-6">
          ...
          <div className="flex gap-4">
            ...
            <button className="border border-black py-2 px-6 rounded-full">Discover More</button>
          </div>
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-4 py-6">
        {['New Arrivals', 'Suit Jackets', 'Co-ords', 'Loungewear', 'Athleisure'].map((item) => (
          <button key={item} className="border border-gray-400 px-4 py-2 rounded-full hover:bg-black hover:text-white">
            {item}
          </button>
        ))}
      </div>
      ...
      <section className="text-center py-8">
        ...
        <div className="flex justify-center gap-4 mt-6">
          {['Casual Wear', 'Loungewear', 'Athletic Wear'].map((cat) => (
            <button key={cat} className="text-gray-500 hover:text-black transition">{cat}</button>
          ))}
        </div>
