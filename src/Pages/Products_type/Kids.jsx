...
const Kids = () => {
  ...
  useEffect(() => {
    ...
    const filtered = allProducts.filter(product => product.type === "kids");
  ...
  const handleFilter = (category) => {
    ...
    if (category === "top") {
      setFilteredProducts(kidsProducts.filter(p => p.category === "top"));
    } else if (category === "bottom") {
      setFilteredProducts(kidsProducts.filter(p => p.category === "bottom"));
    } else if (category === "other") {
      setFilteredProducts(kidsProducts.filter(p => p.category !== "top" && p.category !== "bottom"));
    } else {
  ...
  return (
    <div className="bg-[#D7CCC8] w-full min-h-screen py-6 px-4 sm:px-6">
      <h1 className="text-xl sm:text-2xl text-[#6D4C41] font-bold mb-4 text-center">Kids Wear</h1>
